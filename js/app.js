...
function saveState() {
  try { localStorage.setItem(STATE_KEY, JSON.stringify(window.FOS)); } catch { }
}
...
function navigate(pageId) {
  ...
  document.getElementById('topbar-title').textContent = titles[pageId] || 'Founder OS';
  closeMobileMenu();
  window.scrollTo(0, 0);
...
// ── SVG Line Chart ───────────────────────────────────────────
function buildLineChart(containerId, data, color = '#2563eb', label = '') {
  ...
  el.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:${h}px;">
      ...
      <polyline points="${pts}" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
        style="stroke-dasharray:1000;stroke-dashoffset:1000;animation:drawLine 1.2s ease forwards"/>
      ${xs.map((x, i) => `<circle cx="${x}" cy="${ys[i]}" r="3" fill="${color}" opacity="0.8"/>`).join('')}
    </svg>`;
}
...
function buildRadarChart(containerId, skills) {
  ...
  let axes = entries.map((_, i) => {
    const p = pt(i, 1); return `<line x1="${cx}" y1="${cy}" x2="${p.x}" y2="${p.y}" stroke="rgba(255,255,255,0.07)" stroke-width="1"/>`;
  }).join('');
  const datapts = entries.map(([_, v], i) => { const p = pt(i, v / 100); return `${p.x},${p.y}`; }).join(' ');
  let labels = entries.map(([k], i) => {
    const p = pt(i, 1.2);
    return `<text x="${p.x}" y="${p.y}" text-anchor="middle" dominant-baseline="middle" fill="rgba(255,255,255,0.5)" font-size="9" font-family="Inter,sans-serif">${k}</text>`;
  }).join('');
  el.innerHTML = `
    <svg viewBox="0 0 280 280" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:280px;">
      ${grid}${axes}
      <polygon points="${datapts}" fill="rgba(37,99,235,0.2)" stroke="#3b82f6" stroke-width="2"/>
      ${entries.map(([_, v], i) => { const p = pt(i, v / 100); return `<circle cx="${p.x}" cy="${p.y}" r="4" fill="#3b82f6"/>`; }).join('')}
      ${labels}
    </svg>`;
}
  ...
  el.innerHTML = `<div style="display:flex;align-items:flex-end;gap:6px;height:100px;padding:0 4px">
    ${values.map((v, i) => `
      <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px">
        <span style="font-size:9px;color:rgba(255,255,255,0.4)">${Math.round(v / 1000)}k</span>
        <div style="width:100%;height:${Math.round((v / max) * 80)}px;background:${color};border-radius:3px 3px 0 0;opacity:0.8;transition:opacity .2s" title="${labels[i]}: ${v}"></div>
...
function renderDashboard() {
  ...
  setTimeout(() => {
    ...
    buildLineChart('exec-trend-chart', window.FOS.executionHistory, '#3b82f6', 'Execution');
    buildLineChart('decision-trend', window.FOS.decisionAccuracy, '#10b981', 'Decisions');
    buildLineChart('mrr-trend-chart', window.FOS.mrrHistory, '#06b6d4', 'MRR');
  }, 50);
  // Risk log summary
...
function renderFinance() {
  ...
  const metrics = {
    ...
    'fin-ltvca': (f.ltv / f.cac).toFixed(1) + 'x',
    'fin-churn': f.churn + '%',
  ...
  if (flags) {
    ...
    if (f.ltv / f.cac < 3) alerts.push({ type: 'amber', msg: '⚡ LTV:CAC below 3x — unit economics need improvement' });
...
function updateEnergyDisplay() {
  const v = document.getElementById('energy-slider')?.value || 7;
...
function renderDecisions() {
  ...
  list.innerHTML = d.map((dec, i) => `
    <tr>
      <td><strong style="color:var(--text-primary)">${dec.title}</strong></td>
      <td><span class="badge badge-${dec.confidence > 70 ? 'green' : dec.confidence > 40 ? 'amber' : 'red'}">${dec.confidence}%</span></td>
      <td><span class="badge badge-${dec.reversibility === 'Reversible' ? 'blue' : 'amber'}">${dec.reversibility}</span></td>
      <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${dec.outcome}</td>
      <td>${dec.reviewDate}</td>
      <td><button class="btn btn-ghost btn-sm" onclick="deleteDecision(${i})">✕</button></td>
    </tr>`).join('');
}
...
function submitDecision() {
  ...
  if (!dec.title) return alert('Decision title is required.');
...
function renderLearning() {
  ...
  list.innerHTML = items.map((it, i) => `
    <tr>
      <td><strong style="color:var(--text-primary)">${it.title}</strong></td>
      <td><span class="badge badge-${it.type === 'Book' ? 'blue' : it.type === 'Podcast' ? 'cyan' : it.type === 'Case Study' ? 'purple' : 'green'}">${it.type}</span></td>
      <td><div class="progress-bar" style="width:80px"><div class="progress-fill" style="width:${it.progress}%"></div></div></td>
      <td>${it.progress}%</td>
      <td><button class="btn btn-ghost btn-sm" onclick="deleteLearning(${i})">✕</button></td>
    </tr>`).join('');
}
...
function submitLearning() {
  ...
  if (!it.title) return alert('Title required.');
...
function renderNetwork() {
  ...
  list.innerHTML = contacts.map((c, i) => `
    <tr>
      ...
      <td><span class="badge badge-${c.tier === 'Tier 1' ? 'blue' : c.tier === 'Tier 2' ? 'cyan' : 'purple'}">${c.tier}</span></td>
      <td>${c.role}</td>
      <td>${c.lastContact}</td>
      <td><button class="btn btn-ghost btn-sm" onclick="deleteContact(${i})">✕</button></td>
    </tr>`).join('');
}
    ...
    : goals.map((g, i) => `
        <div class="card mb-sm" style="padding:12px 16px;">
          <div class="flex justify-between items-center">
            <div>
              <div style="font-weight:600;font-size:.9rem">${g.goal}</div>
              <div style="font-size:.8rem;color:var(--text-muted)">${g.quarter} · <span class="badge badge-${g.status === 'On Track' ? 'green' : g.status === 'At Risk' ? 'amber' : 'red'}">${g.status}</span></div>
            </div>
            <button class="btn btn-ghost btn-sm" onclick="deleteGoal(${i})">✕</button>
          </div>
...
function submitGoal() {
  ...
  if (!g.goal) return alert('Goal required.');
...
function submitRisk() {
  ...
  if (!r.title) return alert('Risk title required.');
...
function completeOnboarding() {
  ...
  saveState();
  document.getElementById('sidebar-stage').textContent = window.FOS.founderStage;
  navigate('dashboard');
}
...
function evaluateAdvice() {
  ...
  if (!advice) { alert('Enter the advice first.'); return; }
  ...
  setInner('adv-color-badge', '');
  document.getElementById('adv-score').style.color =
    color === 'green' ? 'var(--accent-green)' : color === 'amber' ? 'var(--accent-amber)' : 'var(--accent-red)';
  setInner('adv-warnings', warnings.length === 0
    ? '✅ No immediate red flags detected.'
    : warnings.map(w => `<div class="alert alert-amber mb-sm">${w}</div>`).join(''));
}
...
document.addEventListener('DOMContentLoaded', () => {
  updateTopbarDate();
  document.getElementById('sidebar-stage').textContent = window.FOS.founderStage;
  document.getElementById('topbar-exec-score').textContent = window.FOS.executionScore + '%';
